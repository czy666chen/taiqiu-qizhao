...
type AuthUser = { id: string; username: string; publicCode: string; nickname: string; avatarUrl: string | null; mustChangePassword: boolean };
...
function SpaLink({ href, navigate, onClick, ...props }: AnchorHTMLAttributes<HTMLAnchorElement> & { href: string; navigate: (path: string) => void }) {
  ...
  return (
    <>
      <header className="app-header">
        <SpaLink className="brand" href="/" navigate={onNavigate} aria-label="返回对局首页">
          <span className="brand-ball">8</span><span><b>台球奇招</b><small>朋友局助手</small></span>
        </SpaLink>
        <nav className="desktop-nav" aria-label="主导航">
          {NAV_ITEMS.map((item) => (
            <SpaLink key={item.path} href={item.path} navigate={onNavigate} className={path === item.path || (item.path !== "/" && path.startsWith(item.path)) ? "active" : ""} aria-current={path === item.path || (item.path !== "/" && path.startsWith(item.path)) ? "page" : undefined}>
        ...
        <div className="header-actions">
          ...
          <SpaLink className="guest-chip" href="/profile" navigate={onNavigate}><span>{user?.nickname.slice(0, 1) || "游"}</span>{user ? user.nickname : "游客模式"}<i className={`sync-dot ${sync.state}`} aria-hidden="true" /></SpaLink>
        </div>
      </header>
...
function EmptyHome({ onStart, onStartEight, onStartSnooker, onStartTeamBattle, onNavigate, onResume, recent, paused, user, onEnterCloudRoom }: { onStart: (mode: MatchMode) => void; onStartEight: () => void; onStartSnooker: () => void; onStartTeamBattle: () => void; onNavigate: (path: string) => void; onResume: (id: string) => void; recent?: BilliardsMatch; paused: BilliardsMatch[]; user: AuthUser | null; onEnterCloudRoom: (code: string) => void }) {
    ...
    <div className="home-page page-shell">
      <section className="welcome-panel">
        <div className="welcome-copy">
          <p className="kicker">MATCH NIGHT · TABLE 08</p>
          <h1>朋友到齐，<br /><em>这局就开。</em></h1>
          <p className="lead">中八、斯诺克、团战、多人追分和奇招牌，一部手机记清比分、单杆与整晚战绩。</p>
          <dl className="hero-proof" aria-label="产品能力">
            <div><dt>5 种</dt><dd>朋友局玩法</dd></div>
            <div><dt>51</dt><dd>张奇招牌</dd></div>
            <div><dt>离线</dt><dd>照样完整记局</dd></div>
          </dl>
          <div className="welcome-actions">
            <button className="primary" onClick={onStartEight}>开始中八比赛 <span aria-hidden="true">→</span></button>
            <SpaLink className="hero-text-link" href="/play" navigate={onNavigate}>先看看所有玩法 <span aria-hidden="true">↗</span></SpaLink>
          </div>
        </div>
        <div className="match-table-preview" aria-label="朋友局计分台示意图">
          <header><span><i /> 现场计分</span><b>第 6 局</b></header>
          <div className="table-score">
            <div><small>小夫</small><strong>4</strong><span>开球</span></div>
            <em>:</em>
            <div><small>儿子</small><strong>3</strong><span>目标 7</span></div>
          </div>
          <div className="felt-table" aria-hidden="true">
            <i className="pocket p1" /><i className="pocket p2" /><i className="pocket p3" /><i className="pocket p4" /><i className="pocket p5" /><i className="pocket p6" />
            <span className="cue-line" /><span className="table-ball cue" /><span className="table-ball eight">8</span><span className="table-ball red" /><span className="table-ball stripe" />
          </div>
          <footer><span>本机自动保存</span><b>最近操作：小夫 +1 局</b></footer>
        </div>
      </section>
      ...
      <section className="quick-start" aria-labelledby="quick-start-title">
        <header className="quick-start-heading"><div><p className="kicker">CHOOSE YOUR GAME</p><h2 id="quick-start-title">今晚玩什么</h2></div></header>
        <div className="quick-grid">
          <button className="quick-card quick-eight" onClick={onStartEight}><span className="quick-index">01</span><span className="quick-icon red" aria-hidden="true">8</span><div><b>中八双人赛</b><small>抢 N 或自由局，逐局记录胜负与犯规</small></div><i aria-hidden="true">↗</i></button>
          <button className="quick-card quick-snooker" onClick={onStartSnooker}><span className="quick-index">02</span><span className="quick-icon snooker" aria-hidden="true">15</span><div><b>标准斯诺克</b><small>15 红、单杆统计与多局赛制</small></div><i aria-hidden="true">↗</i></button>
          <button className="quick-card quick-score" onClick={() => onStart("score")}><span className="quick-index">03</span><span className="quick-icon mint" aria-hidden="true">＋</span><div><b>多人追分</b><small>2–8 人同桌，分值与轮转规则都能改</small></div><i aria-hidden="true">↗</i></button>
          <button className="quick-card quick-team" onClick={onStartTeamBattle}><span className="quick-index">04</span><span className="quick-icon cyan" aria-hidden="true">VS</span><div><b>团战记分</b><small>2–8 人轮换对阵，自动保留每组交手比分</small></div><i aria-hidden="true">↗</i></button>
          <SpaLink className="quick-card quick-room" href="/room" navigate={onNavigate}><span className="quick-index">05</span><span className="quick-icon gold" aria-hidden="true">⇄</span><div><b>多人实时房间</b><small>用房间码邀请球友，多部手机同步操作</small></div><i aria-hidden="true">↗</i></SpaLink>
        </div>
      </section>
      ...
      {recent && (
        <section className="recent-strip">
          ...
          <SpaLink href={`/history/${recent.id}`} navigate={onNavigate}>查看战绩</SpaLink>
        </section>
      )}
    </div>
  );
...
function NonNegativeNumberInput({ ariaLabel, value, max, onValueChange }: { ariaLabel: string; value: number; max?: number; onValueChange: (value: number) => void }) {
  ...
  return <input name={ariaLabel} autoComplete="off" aria-label={ariaLabel} type="number" min="0" max={max} inputMode="numeric" value={draftValue} onChange={(event) => {
...
function SetupDialog({ initialMode, savedRules, scorePresets, onClose, onStart, user, onCloudRoomCreated }: { initialMode: MatchMode; savedRules: ScoreRule[]; scorePresets: ScorePreset[]; onClose: () => void; onStart: (draft: MatchDraft, presets: ScorePreset[]) => void; user: AuthUser | null; onCloudRoomCreated: (code: string) => void }) {
  ...
  const [presetDeletePending, setPresetDeletePending] = useState(false);
  ...
  return (
    <>
      <dialog ref={dialogRef} className="setup-modal" aria-labelledby="setup-title" onCancel={(event) => { event.preventDefault(); onClose(); }}>
          ...
          <div className="setup-body">
            <section className="setup-section">
              ...
              <div className="segmented host-mode-picker">
                <button className={hostMode === "local" ? "active" : ""} aria-pressed={hostMode === "local"} onClick={() => setHostMode("local")}>本机计分</button>
                <button className={hostMode === "cloud" ? "active" : ""} aria-pressed={hostMode === "cloud"} disabled={!user} onClick={() => setHostMode("cloud")}>云端实时房间{!user && <em>登录后可用</em>}</button>
              </div>
            </section>
            <section className="setup-section">
            ...
            {scoreEnabled && (
              <section className="setup-section">
                ...
                <label className="initial-score"><span>统一设置初始积分</span><input name="initial-score" autoComplete="off" type="number" inputMode="numeric" value={initialScore} onChange={(event) => { const value = Number(event.target.value); setInitialScore(value); setPlayerScores(playerScores.map(() => value)); }} /><small>可在玩家姓名右侧单独调整</small></label>
                <div className="turn-strategy"><span>击球顺序</span><div className="segmented"><button className={turnStrategy === "fixed" ? "active" : ""} aria-pressed={turnStrategy === "fixed"} onClick={() => setTurnStrategy("fixed")}>固定轮转</button><button className={turnStrategy === "winner_stays" ? "active" : ""} aria-pressed={turnStrategy === "winner_stays"} onClick={() => setTurnStrategy("winner_stays")}>得分者继续</button></div></div>
                <div className="preset-manager"><select aria-label="计分预设" value={selectedPresetId} onChange={(event) => loadPreset(event.target.value)}><option value={DEFAULT_SCORE_PRESET_ID}>14710 标准（默认）</option><option value="">上次使用规则</option>{presets.map((preset) => <option key={preset.id} value={preset.id}>{preset.name}</option>)}</select><button type="button" disabled={!selectedCustomPreset} onClick={copyPreset}>复制</button><button type="button" disabled={!selectedCustomPreset} onClick={() => setPresetDeletePending(true)}>删除</button></div>
                <div className="rule-editor">
                  {rules.map((rule) => (
                    <label key={rule.id} className={!rule.enabled ? "disabled" : ""}>
                      <input name={`rule-${rule.id}-enabled`} autoComplete="off" type="checkbox" checked={rule.enabled} onChange={(event) => updateRule(rule.id, { enabled: event.target.checked })} />
                      <span className={`rule-dot ${rule.color}`} aria-hidden="true" /><input name={`rule-${rule.id}-name`} autoComplete="off" className="rule-name-input" aria-label={`${rule.label}名称`} value={rule.label} maxLength={12} onChange={(event) => updateRule(rule.id, { label: event.target.value })} /><select aria-label={`${rule.label}颜色`} value={rule.color} onChange={(event) => updateRule(rule.id, { color: event.target.value })}><option value="mint">绿色</option><option value="cyan">青色</option><option value="gold">金色</option><option value="violet">紫色</option><option value="red">红色</option></select><select aria-label={`${rule.label}类型`} value={rule.kind} onChange={(event) => updateRule(rule.id, { kind: event.target.value as "gain" | "penalty" })}><option value="gain">得分</option><option value="penalty">扣分</option></select>
                      <ScoreValueInput key={`${selectedPresetId}-${rule.id}`} rule={rule} onValueChange={(value) => updateRule(rule.id, { value })} />
                      <input name={`rule-${rule.id}-description`} autoComplete="off" className="rule-description-input" aria-label={`${rule.label}说明`} placeholder="例如：第三局翻中袋…" value={rule.description ?? ""} maxLength={40} onChange={(event) => updateRule(rule.id, { description: event.target.value })} /><span className="rule-order"><button type="button" aria-label={`${rule.label}上移`} onClick={() => moveRule(rule.id, -1)}>↑</button><button type="button" aria-label={`${rule.label}下移`} onClick={() => moveRule(rule.id, 1)}>↓</button>{rule.custom && <button type="button" onClick={() => setRules(rules.filter((item) => item.id !== rule.id))}>删除</button>}</span>
                    </label>
                  ))}
                </div>
                <button className="add-player add-score-rule" type="button" onClick={addCustomRule}>＋ 添加自定义计分项</button>
                <div className="save-preset"><label><input name="save-score-preset" autoComplete="off" type="checkbox" checked={savePreset} onChange={(event) => setSavePreset(event.target.checked)} /> {selectedCustomPreset ? "保存对当前预设的编辑" : "另存为命名预设"}</label>{savePreset && <input name="score-preset-name" autoComplete="off" aria-label="计分预设名称" maxLength={24} placeholder="例如：周五俱乐部规则…" value={presetName} onChange={(event) => setPresetName(event.target.value)} />}</div>
              </section>
            )}
            ...
            <section className="setup-section">
              ...
              <div className="segmented card-mode-picker">
                <button className={cardMode === "none" ? "active" : ""} aria-pressed={cardMode === "none"} onClick={() => setCardMode("none")}>不抽牌</button>
                <button className={cardMode === "independent" ? "active" : ""} aria-pressed={cardMode === "independent"} onClick={() => setCardMode("independent")}>独立手牌</button>
              </div>
              {cardMode !== "none" && <><div className="deck-picker" aria-label="选择牌组"><button className={!userDeckId ? "active" : ""} aria-pressed={!userDeckId} onClick={selectOfficialDeck}><b>全量牌库</b><small>{officialDeckCardCount(getOfficialDeck("complete"))} 张 · 官方</small><span>收录全部官方卡牌</span></button>{userDecks.map((deck) => <button key={deck.id} className={userDeckId === deck.id ? "active" : ""} aria-pressed={userDeckId === deck.id} onClick={() => void selectUserDeck(deck.id)}><b>{deck.name}</b><small>V{deck.current_version} · 我的牌组</small><span>已保存到账号</span></button>)}</div><label className="initial-score"><span>每人起始手牌</span><NonNegativeNumberInput ariaLabel="追分每人起始手牌" value={handSize} max={10} onValueChange={setHandSize} /><small>{selectedDeckName} · {selectedDeckCount} 张实体牌</small></label></>}
              {cardMode !== "none" && <div className="advanced-card-settings"><label><span>自动补牌</span><select name="card-auto-draw-policy" autoComplete="off" aria-label="自动补牌策略" value={cardAutoDrawPolicy} onChange={(event) => setCardAutoDrawPolicy(event.target.value as AutoDrawPolicy)}><option value="manual">仅手动抽牌</option><option value="game">每小局补满</option><option value="round">每轮补满</option><option value="after_play">用牌后补一张</option></select></label><label><span>手牌上限</span><input name="card-hand-limit" autoComplete="off" aria-label="手牌上限" type="number" min={handSize} max="20" inputMode="numeric" value={cardHandLimit} onChange={(event) => setCardHandLimit(Number(event.target.value))} /></label><label><span>牌库耗尽</span><select name="card-exhaustion-policy" autoComplete="off" aria-label="牌库耗尽策略" value={cardExhaustionPolicy} onChange={(event) => setCardExhaustionPolicy(event.target.value as DeckExhaustionPolicy)}><option value="stop">停止抽牌</option><option value="reshuffle">确认后重洗弃牌</option></select></label><label><span>最高安全等级</span><select name="card-safety-level" autoComplete="off" aria-label="卡牌最高安全等级" value={maxSafetyLevel} onChange={(event) => setMaxSafetyLevel(event.target.value as "low" | "medium" | "review")}><option value="review">包含待复核</option><option value="medium">排除待复核</option><option value="low">仅低风险</option></select></label><fieldset><legend>排除类别</legend>{([['strategy','竞技策略'],['social','社交惩罚'],['physical','身体动作'],['chaos','趣味混沌']] as const).map(([id, label]) => <label key={id}><input name={`exclude-card-category-${id}`} autoComplete="off" type="checkbox" checked={excludedCategories.includes(id)} onChange={(event) => setExcludedCategories(event.target.checked ? [...excludedCategories, id] : excludedCategories.filter((item) => item !== id))} />{label}</label>)}</fieldset><label className="filter-keywords"><span>排除关键词</span><input name="excluded-card-keywords" autoComplete="off" aria-label="排除卡牌关键词" placeholder="例如：红包，朋友圈…" value={excludedKeywords} onChange={(event) => setExcludedKeywords(event.target.value)} /></label></div>}
            </section>
          </div>
        ) : (
      ...
      {presetDeletePending && selectedCustomPreset && <ConfirmDialog title="删除计分预设？" body={`“${selectedCustomPreset.name}”将从本机预设中移除。`} cancelLabel="保留预设" confirmLabel="确认删除" onCancel={() => setPresetDeletePending(false)} onConfirm={deletePreset} />}
    </>
  );
}
  ...
  return (
    <>
      ...
      <section className="match-section scoring-panel">
        ...
        <label className="score-note"><span>本笔备注</span><input name="score-note" autoComplete="off" aria-label="下一笔计分备注" maxLength={80} placeholder="例如：第三局翻中袋…" value={scoreNote} onChange={(event) => setScoreNote(event.target.value)} /></label>
        <div className="transfer-entry"><button className="text-button" onClick={() => setTransferOpen(!transferOpen)}>⇄ {transferOpen ? "收起转账计分" : "转账计分"}</button>{transferOpen && <div className="transfer-panel"><header><b>获胜者：{match.players.find((player) => player.id === selectedId)?.name}</b><small>每名所选输家支付同样分数，总分保持不变</small></header><div className="transfer-losers">{rankings.filter((player) => player.id !== selectedId).map((player) => <label key={player.id}><input name={`transfer-loser-${player.id}`} autoComplete="off" type="checkbox" checked={transferLosers.includes(player.id)} onChange={(event) => setTransferLosers(event.target.checked ? [...transferLosers, player.id] : transferLosers.filter((id) => id !== player.id))} />{player.name}</label>)}</div><div className="transfer-fields"><label><span>每人支付</span><input name="transfer-amount" autoComplete="off" aria-label="每名输家支付分数" type="number" min="1" inputMode="numeric" value={transferAmount} onChange={(event) => setTransferAmount(Number(event.target.value))} /></label><label><span>备注</span><input name="transfer-note" autoComplete="off" aria-label="转账计分备注" maxLength={80} placeholder="例如：第三局结算…" value={transferNote} onChange={(event) => setTransferNote(event.target.value)} /></label><button disabled={!transferLosers.length || transferAmount <= 0} onClick={() => { onTransfer(selectedId, transferLosers, transferAmount, transferNote); setTransferLosers([]); setTransferNote(""); setManualSelectedId(null); setTransferOpen(false); }}>确认转账</button></div></div>}</div>
        <div className="transfer-entry"><button className="text-button" onClick={() => setBackfillOpen(!backfillOpen)}>＋ {backfillOpen ? "收起补录" : "补录计分事件"}</button>{backfillOpen && <div className="backfill-panel"><label><span>原因</span><input name="backfill-label" autoComplete="off" aria-label="补录原因" maxLength={30} placeholder="例如：漏记犯规…" value={backfillLabel} onChange={(event) => setBackfillLabel(event.target.value)} /></label><label><span>分值（可负数）</span><input name="backfill-delta" autoComplete="off" aria-label="补录分值" type="number" inputMode="numeric" value={backfillDelta} onChange={(event) => setBackfillDelta(Number(event.target.value))} /></label><label><span>备注</span><input name="backfill-note" autoComplete="off" aria-label="补录备注" maxLength={80} placeholder="例如：依据纸质记录补录…" value={backfillNote} onChange={(event) => setBackfillNote(event.target.value)} /></label><button disabled={!backfillLabel.trim() || backfillDelta === 0} onClick={() => { onBackfill(selectedId, backfillDelta, backfillLabel, backfillNote); setBackfillDelta(0); setBackfillLabel(""); setBackfillNote(""); setBackfillOpen(false); }}>确认补录</button></div>}</div>
        <div className="transfer-entry"><button className="text-button" onClick={() => setSpecialOpen(!specialOpen)}>◆ {specialOpen ? "收起特殊规则" : "黑金 / 让杆"}</button>{specialOpen && <div className="special-score-panel"><label><span>基础分</span><input name="special-score-amount" autoComplete="off" aria-label="特殊规则基础分" type="number" min="1" inputMode="numeric" value={specialAmount} onChange={(event) => setSpecialAmount(Number(event.target.value))} /></label><label><span>让分方</span><select name="handicap-grantor" autoComplete="off" aria-label="让杆让分方" value={handicapGrantorId} onChange={(event) => setHandicapGrantorId(event.target.value)}><option value="">请选择</option>{rankings.filter((player) => player.id !== selectedId).map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</select></label><label><span>备注</span><input name="special-score-note" autoComplete="off" aria-label="特殊规则备注" maxLength={80} placeholder="例如：赛前约定…" value={specialNote} onChange={(event) => setSpecialNote(event.target.value)} /></label><button disabled={specialAmount <= 0} onClick={() => { onBlackGold(selectedId, specialAmount, specialNote); setSpecialNote(""); setSpecialOpen(false); }}>黑金结算（每家 ×2）</button><button disabled={specialAmount <= 0 || !handicapGrantorId} onClick={() => { onHandicap(selectedId, handicapGrantorId, specialAmount, specialNote); setSpecialNote(""); setHandicapGrantorId(""); setSpecialOpen(false); }}>记录让杆</button></div>}</div>
...
function CardBoard({ match, onChange, toast }: { match: BilliardsMatch; onChange: (match: BilliardsMatch) => void; toast: (message: string) => void }) {
  ...
  return (
    <section className="match-section card-board">
      ...
      <div className="card-control-panel"><label><span>补牌策略</span><select name="match-card-auto-draw-policy" autoComplete="off" aria-label="对局中自动补牌策略" value={cards.autoDrawPolicy ?? "manual"} onChange={(event) => onChange(updateMatchCardSettings(match, { autoDrawPolicy: event.target.value as AutoDrawPolicy }))}><option value="manual">仅手动</option><option value="game">每小局</option><option value="round">每轮</option><option value="after_play">用牌后</option></select></label><label><span>手牌上限</span><input name="match-card-hand-limit" autoComplete="off" aria-label="对局中手牌上限" type="number" min={cards.initialHandSize} max="20" inputMode="numeric" value={handLimit} onChange={(event) => onChange(updateMatchCardSettings(match, { handLimit: Number(event.target.value) }))} /></label><label><span>耗尽策略</span><select name="match-card-exhaustion-policy" autoComplete="off" aria-label="对局中牌库耗尽策略" value={cards.exhaustionPolicy ?? "stop"} onChange={(event) => onChange(updateMatchCardSettings(match, { exhaustionPolicy: event.target.value as DeckExhaustionPolicy }))}><option value="stop">停止抽牌</option><option value="reshuffle">确认后重洗</option></select></label><button disabled={cards.autoDrawPolicy !== "game"} onClick={() => triggerRefill("game")}>{reshuffleArmed ? "确认重洗并补满" : "新小局补牌"}</button><button disabled={cards.autoDrawPolicy !== "round"} onClick={() => triggerRefill("round")}>{reshuffleArmed ? "确认重洗并补满" : "新一轮补牌"}</button></div>
      {handIds.length > 1 && <div className="hand-tabs">{handIds.map((id) => <button key={id} className={activeHand === id ? "active" : ""} aria-pressed={activeHand === id} onClick={() => setHandId(id)}>{match.players.find((player) => player.id === id)?.name}<small>{cards.hands[id].length} 张</small></button>)}</div>}
      <div className="card-score-link"><header><b>卡牌影响积分（可选）</b><small>填写非零分值后，使用卡牌会生成双向关联计分事件。</small></header><select name="card-score-player" autoComplete="off" aria-label="卡牌计分玩家" value={linkPlayerId} onChange={(event) => setLinkPlayerId(event.target.value)}>{match.players.filter((player) => player.active).map((player) => <option key={player.id} value={player.id}>{player.name}</option>)}</select><input name="card-score-delta" autoComplete="off" aria-label="卡牌关联分值" type="number" inputMode="numeric" value={linkDelta} onChange={(event) => setLinkDelta(Number(event.target.value))} /><input name="card-score-note" autoComplete="off" aria-label="卡牌关联计分备注" maxLength={80} placeholder="例如：卡牌奖励分…" value={linkNote} onChange={(event) => setLinkNote(event.target.value)} /></div>
...
function ActiveMatchView({ match, readOnly = false, onChange, onFinish, toast }: { match: BilliardsMatch; readOnly?: boolean; onChange: (match: BilliardsMatch) => void; onFinish: () => void; toast: (message: string) => void }) {
  ...
  return (
    <div className={`match-page page-shell${readOnly ? " read-only-match" : ""}`}>
      ...
      <div className="match-dock"><button disabled={!match.scoreEvents.length} onClick={() => onChange(undoLastScore(match))}>↶<span>撤销</span></button><button className="dock-main" onClick={() => match.cards ? document.querySelector(".card-board")?.scrollIntoView({ behavior: preferredScrollBehavior() }) : document.querySelector(".scoring-panel")?.scrollIntoView({ behavior: preferredScrollBehavior() })}>{match.cards ? "抽牌" : "记分"}</button><button onClick={() => setMoreOpen(!moreOpen)}>•••<span>更多</span></button></div>
    </div>
  );
}
...
type CustomCardRow = { id: string; title: string; effect: string; default_quantity: number; safety_level: "low" | "medium" | "review"; safety_note: string | null; supportedGames: Array<"chinese_eight" | "snooker"> };
...
function DecksPage({ user }: { user: AuthUser | null }) {
  ...
  const [catalogKind, setCatalogKind] = useState<"all" | "snooker" | null>(null);
  ...
  const [cardSupportsSnooker, setCardSupportsSnooker] = useState(false);
  ...
  const snookerCards = CARD_DEFINITIONS.filter((card) => card.supportedGames.includes("snooker"));
  const cards = CARD_DEFINITIONS.filter((card) => (catalogKind !== "snooker" || card.supportedGames.includes("snooker")) && `${card.title}${card.effect}`.toLowerCase().includes(query.trim().toLowerCase()));
  ...
  const saveCard = async () => {
    await apiPayload(await fetch(editingCardId ? `/api/custom-cards/${editingCardId}` : "/api/custom-cards", { method: editingCardId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title: cardTitle, effect: cardEffect, safetyLevel: cardSafetyLevel, safetyNote: cardSafetyNote, supportedGames: ["chinese_eight", ...(cardSupportsSnooker ? ["snooker"] : [])], defaultQuantity: 1 }) }));
    setCardTitle(""); setCardEffect(""); setCardSafetyLevel("low"); setCardSafetyNote(""); setCardSupportsSnooker(false); setEditingCardId(""); await refresh(); setMessage(editingCardId ? "自定义卡牌已更新" : "自定义卡牌已创建");
  };
  const remove = async () => {
    if (!pendingDelete || deleteBusy) return;
    setDeleteBusy(true); setDeleteError("");
    try {
      await apiPayload(await fetch(pendingDelete.path, { method: "DELETE" }));
      await refresh();
      setMessage(`${pendingDelete.label}已删除；历史版本快照保持不变`);
  ...
  return <div className="content-page page-shell">
    <header className="page-title split"><div><p className="kicker">DECK LIBRARY</p><h1>牌组</h1></div><div className="deck-summary"><span>2<small>官方牌组</small></span><span>{decks.length}<small>我的牌组</small></span></div></header>
    <div className="official-deck-grid"><button type="button" className="official-deck" aria-haspopup="dialog" onClick={() => setCatalogKind("all")}><div className="official-art"><span aria-hidden="true">8</span></div><div><p className="kicker">OFFICIAL · V1</p><span className="official-deck-title">全量牌库</span><p>{official.description}</p><div className="tag-row"><span>{officialDeckCardCount(official)} 张</span><span>全部玩法</span></div></div></button><button type="button" className="official-deck" aria-haspopup="dialog" onClick={() => setCatalogKind("snooker")}><div className="official-art"><span aria-hidden="true">15</span></div><div><p className="kicker">SNOOKER · V1</p><span className="official-deck-title">斯诺克牌组</span><p>适配标准 15 红斯诺克，保留球值、合法击球顺序与胜负条件。</p><div className="tag-row"><span>{snookerCards.reduce((sum, card) => sum + card.count, 0)} 张</span><span>斯诺克兼容</span></div></div></button></div>
    {user ? <>
      <section className="card-catalog deck-editor"><div className="section-heading"><div><p className="kicker">MY DECKS</p><h2>{editingId ? "编辑牌组" : "新建牌组"}</h2></div>{editingId && <button onClick={resetEditor}>取消编辑</button>}</div>
        <label><span>牌组名称</span><input name="deck-name" autoComplete="off" maxLength={40} value={deckName} onChange={(event) => setDeckName(event.target.value)} /></label>
        <div className="deck-list">{decks.map((deck) => <article key={deck.id}><button type="button" className="deck-list-item-main" onClick={() => void editDeck(deck.id)}><b>{deck.name}</b><small>V{deck.current_version}</small></button><button type="button" className="deck-list-action" aria-label={`复制${deck.name}`} onClick={() => void copyDeck(deck.id)}>复制</button><button type="button" className="deck-list-action is-danger" aria-label={`删除${deck.name}`} onClick={() => { setDeleteError(""); setPendingDelete({ path: `/api/decks/${deck.id}`, label: `牌组“${deck.name}”` }); }}>删除</button></article>)}</div>
        <div className="catalog-list deck-card-picker virtualized-list">{cards.map((card) => { const key = `official:${card.id}`; return <article key={key}><input name={`deck-card-${card.id}-quantity`} autoComplete="off" aria-label={`${card.title}数量`} type="number" min="0" max="10" inputMode="numeric" value={quantities[key] ?? 0} onChange={(event) => setQuantity(key, Number(event.target.value))} /><div><b>{card.title}</b><p>{card.effect}</p></div></article>; })}{customCards.map((card) => { const key = `custom:${card.id}`; return <article key={key}><input name={`custom-card-${card.id}-quantity`} autoComplete="off" aria-label={`${card.title}数量`} type="number" min="0" max="10" inputMode="numeric" value={quantities[key] ?? 0} onChange={(event) => setQuantity(key, Number(event.target.value))} /><div><b>{card.title} <em>我的卡牌</em></b><p>{card.effect}</p></div></article>; })}</div>
      ...
      <section className="card-catalog custom-card-editor"><div className="section-heading"><div><p className="kicker">MY CARDS</p><h2>{editingCardId ? "编辑卡牌" : "我的卡牌"}</h2></div>{editingCardId && <button onClick={() => { setEditingCardId(""); setCardTitle(""); setCardEffect(""); setCardSafetyNote(""); setCardSupportsSnooker(false); }}>取消编辑</button>}</div><div className="eight-form-grid"><label className="wide"><span>标题</span><input name="custom-card-title" autoComplete="off" maxLength={40} value={cardTitle} onChange={(event) => setCardTitle(event.target.value)} /></label><label className="wide"><span>效果</span><input name="custom-card-effect" autoComplete="off" maxLength={500} value={cardEffect} onChange={(event) => setCardEffect(event.target.value)} /></label><label className="wide custom-card-snooker-confirm"><input name="custom-card-supports-snooker" autoComplete="off" type="checkbox" checked={cardSupportsSnooker} onChange={(event) => setCardSupportsSnooker(event.target.checked)} /><span>我已确认此牌不改变斯诺克球值、合法顺序、台面复位或胜负条件</span></label></div><button disabled={!cardTitle.trim() || !cardEffect.trim()} onClick={() => void saveCard().catch((error) => setMessage(error instanceof Error ? error.message : "保存失败"))}>{editingCardId ? "保存卡牌" : "创建卡牌"}</button><div className="deck-list">{customCards.map((card) => <article key={card.id}><button type="button" className="deck-list-item-main" onClick={() => { setEditingCardId(card.id); setCardTitle(card.title); setCardEffect(card.effect); setCardSafetyLevel(card.safety_level); setCardSafetyNote(card.safety_note ?? ""); setCardSupportsSnooker(card.supportedGames.includes("snooker")); }}><b>{card.title}</b><small>{card.effect}{card.supportedGames.includes("snooker") ? " · 兼容斯诺克" : ""}</small></button><button type="button" className="deck-list-action is-danger" aria-label={`删除${card.title}`} onClick={() => { setDeleteError(""); setPendingDelete({ path: `/api/custom-cards/${card.id}`, label: `卡牌“${card.title}”` }); }}>删除</button></article>)}</div></section>
      {message && <p className="form-message" role="status">{message}</p>}
    </> : <section className="card-catalog"><h2>登录后创建自己的卡牌与牌组</h2><p className="form-message">官方全量牌库无需登录即可使用。</p></section>}
    {catalogKind && <dialog ref={catalogDialogRef} className="setup-modal card-catalog-modal" aria-labelledby="official-catalog-title" onClose={() => setCatalogKind(null)}><header className="modal-heading"><div><p className="kicker">{catalogKind === "snooker" ? "SNOOKER COMPATIBLE CARDS" : "ALL OFFICIAL CARDS"}</p><h2 id="official-catalog-title">{catalogKind === "snooker" ? "斯诺克牌组清单" : "官方卡牌清单"}</h2></div><button type="button" className="icon-button" aria-label="关闭官方卡牌清单" onClick={() => catalogDialogRef.current?.close()}>×</button></header><div className="catalog-modal-body"><label className="search"><span aria-hidden="true">⌕</span><input name="official-card-search" autoComplete="off" aria-label="搜索官方卡牌" type="search" placeholder="例如：安全挑战…" value={query} onChange={(event) => setQuery(event.target.value)} /></label><div className="catalog-list virtualized-list">{cards.map((card) => <article key={card.id}><span>{card.id.slice(-3)}</span><div><b>{card.title}{card.count > 1 && <em> ×{card.count}</em>}</b><p>{card.effect}</p></div></article>)}</div></div></dialog>}
    {pendingDelete && <ConfirmDialog title="确认删除？" body={`${pendingDelete.label}将立即删除，但历史对局中的版本快照会保留。`} cancelLabel="取消" confirmLabel="确认删除" busy={deleteBusy} error={deleteError} onCancel={() => { setPendingDelete(null); setDeleteError(""); }} onConfirm={() => void remove()} />}
  </div>;
}
...
function EightBallSetupDialog({ defaultLayout, onClose, onStart, user, onCloudRoomCreated }: { defaultLayout: EightBallLayout; onClose: () => void; onStart: (draft: EightBallDraft) => void; user: AuthUser | null; onCloudRoomCreated: (code: string) => void }) {
  ...
  return (
    <dialog ref={dialogRef} className="setup-modal eight-setup" aria-labelledby="eight-setup-title" onCancel={(event) => { event.preventDefault(); onClose(); }}>
        <header className="modal-heading">
          <div><p className="kicker">CHINESE EIGHT · NEW MATCH</p><h2 id="eight-setup-title">创建中八比赛</h2></div>
          <button className="icon-button" aria-label="关闭中八比赛设置" onClick={onClose}>×</button>
        </header>
        <div className="setup-body">
          <section className="setup-section">
            <div className="setup-title"><span>00</span><div><b>对局方式</b><small>本机计分离线可用；云端实时房间仅登录用户可用</small></div></div>
            ...
            <div className="player-inputs">
              {names.map((name, index) => (
                <label className="player-input" key={index}>
                  <span>{index ? "红" : "蓝"}</span>
                  <input name={`eight-ball-player-${index + 1}-name`} autoComplete="off" aria-label={`中八玩家 ${index + 1} 姓名`} maxLength={16} value={name} onChange={(event) => setNames(names.map((item, itemIndex) => itemIndex === index ? event.target.value : item) as [string, string])} />
                </label>
              ))}
            </div>
          </section>
          <section className="setup-section">
            ...
            <div className="eight-form-grid">
              ...
              {raceMode === "race" && <label><span>抢几局</span><input name="eight-ball-race-to" autoComplete="off" type="number" min="1" max="99" value={raceTo} onChange={(event) => setRaceTo(Number(event.target.value))} /></label>}
            </div>
          </section>
          <section className="setup-section">
            <div className="setup-title"><span>03</span><div><b>比赛资料</b><small>全部可选，将进入战绩导出</small></div></div>
            <div className="eight-form-grid">
              <label><span>比赛名称</span><input name="eight-ball-title" autoComplete="off" maxLength={40} value={title} onChange={(event) => setTitle(event.target.value)} /></label>
              <label><span>地点</span><input name="eight-ball-location" autoComplete="off" maxLength={40} value={location} onChange={(event) => setLocation(event.target.value)} /></label>
              ...
              <label className="wide"><span>备注</span><input name="eight-ball-note" autoComplete="off" maxLength={120} placeholder="例如：布局可在计分板内切换…" value={note} onChange={(event) => setNote(event.target.value)} /></label>
            </div>
          </section>
          <section className="setup-section">
...
function exportSnookerJson(match: SnookerMatch) {
  ...
  downloadText(`斯诺克战绩-${match.id}.json`, JSON.stringify({
    exportVersion: 2,
    exportedAt: new Date().toISOString(),
    match,
    matchScore: Object.fromEntries(match.players.map((player) => [player.id, match.framesWon[player.id]])),
    frameScores: match.completedFrames.map((frame) => ({ number: frame.number, scores: frame.scores, winnerId: frame.winnerId, endReason: frame.endReason })),
    stats,
...
function UnifiedHistoryPage({ history, eightBallHistory, snookerHistory, teamBattleHistory, selectedId, onSelect, onDeleteMatch }: { history: BilliardsMatch[]; eightBallHistory: EightBallMatch[]; snookerHistory: SnookerMatch[]; teamBattleHistory: TeamBattleMatch[]; selectedId?: string; onSelect: (id: string) => void; onDeleteMatch: (id: string) => void }) {
  ...
  const openHistory = (event: MouseEvent<HTMLAnchorElement>, id: string) => {
  ...
  if (!selected) {
    ...
    return <div className="content-page page-shell"><header className="page-title"><p className="kicker">MATCH HISTORY</p><h1>战绩</h1><p>团战、斯诺克、中八与追分使用统一的战绩入口。</p></header>{all.length ? <div className="history-grid">{all.map((item) => { if (item.kind === "team") { const match = item.match; const projection = getTeamBattleProjection(match); const leader = projection.standings[0]; return <div className="history-card" key={match.id}><a className="history-card-main" href={`/history/${match.id}`} onClick={(event) => openHistory(event, match.id)}><span className="history-type team">团战记分</span><b>{match.players.map((player) => player.name).join(" · ")}</b><small>{formatTime(match.startedAt)} · {durationLabel(teamBattleElapsedMs(match, match.endedAt))}</small><div><span>总览</span><strong>{leader ? `${leader.player.name} 暂列第 1 · ` : ""}{projection.rounds.length} 局 · {projection.pairs.length} 组交手</strong><i aria-hidden="true">→</i></div></a><button className="history-delete" onClick={() => onDeleteMatch(match.id)}>删除</button></div>; } if (item.kind === "snooker") { const match = item.match; const stats = getSnookerBreakStats(match); const winner = [...match.players].sort((a, b) => match.framesWon[b.id] - match.framesWon[a.id])[0]; return <div className="history-card" key={match.id}><a className="history-card-main" href={`/history/${match.id}`} onClick={(event) => openHistory(event, match.id)}><span className="history-type snooker">斯诺克</span><b>{match.players.map((player) => player.name).join(" · ")}</b><small>{formatTime(match.startedAt)} · {durationLabel(snookerElapsedMs(match))}</small><div><span>局分</span><strong>{winner.name} · {match.players.map((player) => match.framesWon[player.id]).join(" : ")} · 最高单杆 {stats.highestBreak}</strong><i aria-hidden="true">→</i></div></a><button className="history-delete" onClick={() => onDeleteMatch(match.id)}>删除</button></div>; } if (item.kind === "eight") { const match = item.match; const stats = calculateEightBallStats(match); const winner = [...match.players].sort((a, b) => stats[b.id].score - stats[a.id].score)[0]; return <div className="history-card" key={match.id}><a className="history-card-main" href={`/history/${match.id}`} onClick={(event) => openHistory(event, match.id)}><span className="history-type eight">中八双人赛</span><b>{match.players.map((player) => player.name).join(" · ")}</b><small>{formatTime(match.startedAt)} · {durationLabel(eightBallElapsedMs(match))}</small><div><span>获胜者</span><strong>{winner.name} · {match.players.map((p) => stats[p.id].score).join(" : ")}</strong><i aria-hidden="true">→</i></div></a><button className="history-delete" onClick={() => onDeleteMatch(match.id)}>删除</button></div>; } const match = item.match; const winner = getRankings(match)[0]; return <div className="history-card" key={match.id}><a className="history-card-main" href={`/history/${match.id}`} onClick={(event) => openHistory(event, match.id)}><span className="history-type">{match.mode === "cards" ? "奇招牌" : match.mode === "score_cards" ? "追分 + 奇招牌" : "多人追分"}</span><b>{match.players.map((player) => player.name).join(" · ")}</b><small>{formatTime(match.startedAt)} · {formatDuration(match.startedAt, match.endedAt)}</small><div><span>第一名</span><strong>{winner?.name}{match.mode !== "cards" && ` · ${winner?.score} 分`}</strong><i aria-hidden="true">→</i></div></a><button className="history-delete" onClick={() => onDeleteMatch(match.id)}>删除</button></div>; })}</div> : <div className="large-empty"><span aria-hidden="true">⌁</span><h2>还没有战绩</h2><p>完成第一场比赛后，逐局流水会保存在这里。</p></div>}</div>;
  }
  ...
  const jumpTo = (id: string) => document.getElementById(`timeline-${id}`)?.scrollIntoView({ behavior: preferredScrollBehavior(), block: "center" });
  // This link delegates unmodified clicks to the app's in-memory router while
  // preserving a real href for open-in-new-tab and copied-link behavior.
  // eslint-disable-next-line @next/next/no-html-link-for-pages
  return <div className="content-page page-shell"><a className="back-link" href="/history" onClick={(event) => openHistory(event, "")}>← 返回战绩</a><header className="page-title split"><div><p className="kicker">MATCH DETAIL</p><h1>{selected.mode === "cards" ? "奇招卡牌局" : "追分结算"}</h1><p>{formatTime(selected.startedAt)} · {formatDuration(selected.startedAt, selected.endedAt)}</p></div><div className="report-actions"><ReportControls options={reportOptions} onChange={setReportOptions} onImage={() => exportScoreImage(selected, reportOptions)} onPdf={() => printScoreMatch(selected, reportOptions)} /><div className="export-actions minor"><button onClick={() => exportScoreJson(selected)}>JSON 备份</button><button className="danger-text" onClick={() => onDeleteMatch(selected.id)}>删除战绩</button></div></div></header><section className="result-podium">{rankings.map((player, index) => <div key={player.id}><span>{index + 1}</span><b>{player.name}</b><strong>{player.score}<small> 分</small></strong><small>较开局 {player.score - player.initialScore >= 0 ? "+" : ""}{player.score - player.initialScore}</small></div>)}</section><section className="event-stats"><div><strong>{selected.scoreEvents.length}</strong><span>计分事件</span></div><div><strong>{selected.cards?.events.length ?? 0}</strong><span>卡牌事件</span></div><div><strong>{timeline.filter((event) => event.linkedId).length / 2}</strong><span>牌分联动</span></div></section><section className="history-detail"><div className="section-heading"><div><p className="kicker">UNIFIED TIMELINE</p><h2>真实发生顺序</h2></div></div>{timeline.map((event) => <div className={`timeline-row unified ${event.kind}`} id={`timeline-${event.id}`} key={event.id}><span>{formatTime(event.at)}</span><div><b><i>{event.kind === "score" ? "积分" : "卡牌"}</i>{event.label}</b>{event.note && <small>{event.note}</small>}{event.linkedId && <button onClick={() => jumpTo(event.linkedId!)}>查看关联{event.kind === "score" ? "卡牌" : "积分"} ↕</button>}</div>{event.value !== undefined && <strong className={event.value < 0 ? "negative" : "positive"}>{event.value > 0 ? "+" : ""}{event.value}</strong>}</div>)}</section></div>;
}
...
function AccountForm({ onAuthenticated }: { onAuthenticated: (user: AuthUser) => void }) {
  ...
  const [message, setMessage] = useState("");
  useEffect(() => {
    const syncMode = () => setMode(accountModeFromLocation());
  ...
  return <section className="account-panel"><div className="segmented account-mode-switch"><button type="button" className={mode === "login" ? "active" : ""} aria-pressed={mode === "login"} onClick={() => selectMode("login")}>登录</button><button type="button" className={mode === "register" ? "active" : ""} aria-pressed={mode === "register"} onClick={() => selectMode("register")}>邀请码注册</button></div><form onSubmit={(event) => { event.preventDefault(); void submit(); }}><label>用户名<input name="username" autoComplete="username" spellCheck={false} maxLength={24} required value={username} onChange={(event) => setUsername(event.target.value)} />{mode === "register" && <small>{USERNAME_HELP_TEXT}</small>}</label>{mode === "register" && <label>昵称<input name="nickname" autoComplete="nickname" maxLength={40} value={nickname} onChange={(event) => setNickname(event.target.value)} /></label>}<label>密码<input name="password" type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} minLength={6} maxLength={128} required value={password} onChange={(event) => setPassword(event.target.value)} /></label>{mode === "register" && <label>固定邀请码<input name="invite-code" type="password" autoComplete="off" spellCheck={false} required value={inviteCode} onChange={(event) => setInviteCode(event.target.value)} /></label>}<button className="primary" disabled={busy}>{busy ? "正在连接…" : mode === "login" ? "登录并恢复同步" : "创建账号"}</button></form>{message && <p className="form-message" role="alert">{message}</p>}</section>;
}
  ...
  return <section className="realtime-eight-board">
    ...
    <div className="realtime-eight-score">
      {match.players.map((player, index) => {
        ...
        return <article className={index ? "blue" : "red"} key={player.id}>
          <span>{player.nickname}</span><strong aria-live="polite" aria-atomic="true">{stats.score}</strong>
    ...
    <div className="realtime-eight-form">
      <div className="eight-winner-picker">
        {match.players.map((player) => <button disabled={disabled} className={winnerId === player.id ? "active" : ""} aria-pressed={winnerId === player.id} key={player.id} onClick={() => setWinnerId(player.id)}>{player.nickname}获胜</button>)}
      </div>
      <div className="segmented">
        {Object.entries(EIGHT_BALL_WIN_LABELS).map(([id, label]) => <button disabled={disabled} className={winType === id ? "active" : ""} aria-pressed={winType === id} key={id} onClick={() => setWinType(id as EightBallWinType)}>{label}</button>)}
      </div>
      <div className="eight-fouls">
        {match.players.map((player) => <label key={player.id}><span>{player.nickname}本局犯规</span><input name={`realtime-eight-${player.id}-fouls`} autoComplete="off" disabled={disabled} type="number" min={0} max={99} inputMode="numeric" value={fouls[player.id] ?? 0} onChange={(event) => setFouls({ ...fouls, [player.id]: Number(event.target.value) })} /></label>)}
      </div>
      <label className="realtime-note">本局备注<input name="realtime-eight-round-note" autoComplete="off" disabled={disabled} maxLength={120} value={note} onChange={(event) => setNote(event.target.value)} placeholder="例如：关键球处理…" /></label>
...
function SnookerHistoryReportDetail({ match, onBack, onDelete }: { match: SnookerMatch; onBack: () => void; onDelete: (id: string) => void }) {
  const [reportOptions, setReportOptions] = useState(DEFAULT_REPORT_OPTIONS);
  const actions = <div className="report-actions"><ReportControls options={reportOptions} onChange={setReportOptions} onImage={() => exportSnookerImage(match, reportOptions)} onPdf={() => printSnooker(match, reportOptions)} /><div className="export-actions minor"><button onClick={() => exportSnookerJson(match)}>JSON 备份</button><button className="danger-text" onClick={() => onDelete(match.id)}>删除战绩</button></div></div>;
  return <SnookerHistoryDetail match={match} onBack={onBack} reportActions={actions} />;
}

const REALTIME_SNOOKER_BALLS = Object.entries(SNOOKER_BALL_VALUES) as Array<[SnookerBall, number]>;
const REALTIME_SNOOKER_LABELS: Record<SnookerBall, string> = { red: "红", yellow: "黄", green: "绿", brown: "棕", blue: "蓝", pink: "粉", black: "黑" };

type RealtimeSnookerLeadNotice = {
  frameId: string;
  frameNumber: number;
  leaderName: string;
  trailingName: string;
...
function RealtimeSnookerLeadDialog({ notice, onDismiss }: { notice: RealtimeSnookerLeadNotice; onDismiss: () => void }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  useEffect(() => { dialogRef.current?.showModal(); }, []);
  return <dialog ref={dialogRef} className="snooker-lead-dialog" role="alertdialog" aria-labelledby="snooker-lead-title" aria-describedby="snooker-lead-description" onCancel={(event) => { event.preventDefault(); onDismiss(); }}>
    <small>第 {notice.frameNumber} 局 · 超分提醒</small>
    <h2 id="snooker-lead-title">{notice.leaderName} 已超分</h2>
    <strong>超分 {notice.excessPoints} 分</strong>
    <p id="snooker-lead-description">{notice.trailingName} 落后 {notice.lead} 分，台面剩余最高 {notice.remainingPoints} 分。</p>
    <button type="button" className="primary" autoFocus onClick={onDismiss}>知道了</button>
  </dialog>;
}

function RealtimeSnookerPanel({ snapshot, writable, busy, onCommand, isHost }: {
  snapshot: RealtimeSnapshot;
  writable: boolean;
  busy: boolean;
  onCommand: (kind: string, payload: RealtimeCommandPayload) => void;
  ...
  const disabled = busy || !writable || !!match.pausedAt;
  const legal = (ball: SnookerBall) => {
    if (disabled || frame.status !== "active") return false;
  ...
  const striker = state.players.find((player) => player.id === frame.strikerId)!;
  const targetReached = match.bestOf !== null && Object.values(match.framesWon).some((wins) => wins >= Math.ceil(match.bestOf! / 2));
  return <section className="realtime-score-board realtime-snooker-board">
    <header><div><p className="kicker">SNOOKER · SERVER AUTHORITY</p><h3>实时斯诺克</h3><small>{match.bestOf ? `Best of ${match.bestOf}` : "自由局"} · {match.initialReds} 红 · 当前第 {frame.number} 局 · {match.variant === "trick_cards" ? "奇招牌变体" : match.initialReds === 15 ? "标准规则" : "自定义红球局"}</small></div><button className="secondary" disabled={disabled || !match.events.length} onClick={() => onCommand("snooker.event.undo", {})}>↶ 撤销上一球</button></header>
    {targetReached && <p className="target-notice">已达到获胜局数，请由房主核对后结束房间。</p>}
    <div className="realtime-eight-score">{state.players.map((player, index) => <article className={index ? "blue" : "red"} key={player.id}><span>{player.nickname}{player.id === frame.strikerId ? " · 当前击球" : ""}</span><strong>{frame.scores[player.id]}</strong><small>局分 {match.framesWon[player.id]} · 本局最高 {Math.max(0, ...frame.breaks.filter((item) => item.playerId === player.id).map((item) => item.points), frame.currentBreak.playerId === player.id ? frame.currentBreak.points : 0)}</small></article>)}</div>
    <div className="snooker-break-line"><div><span>当前单杆</span><strong>{frame.currentBreak.points}</strong></div><div><span>剩余红球</span><strong>{frame.redsRemaining}</strong></div><p><b>{striker.nickname}</b> · {frame.phase === "colour_after_red" ? "任意彩球" : frame.phase === "reds" ? "目标红球" : `目标${REALTIME_SNOOKER_LABELS[frame.nextColour ?? "black"]}球`}</p></div>
    <div className="snooker-ball-row">{REALTIME_SNOOKER_BALLS.map(([ball, points]) => <button key={ball} className={`snooker-ball ${ball}`} disabled={!legal(ball)} onClick={() => onCommand("snooker.pot.record", { ball })}><span>{points}</span><b>{REALTIME_SNOOKER_LABELS[ball]}</b></button>)}</div>
    <div className="snooker-primary-actions"><button className="primary" disabled={disabled || frame.status !== "active"} onClick={() => onCommand("snooker.visit.end", { reason: "miss" })}>未进／结束本杆</button>{isHost && [4, 5, 6, 7].map((points) => <button key={points} disabled={disabled || frame.status !== "active"} onClick={() => onCommand("snooker.foul.record", { values: [points] })}>犯规 +{points}</button>)}</div>
    {isHost && frame.status === "completed" && <button className="primary" disabled={busy} onClick={() => onCommand("snooker.frame.finish", { winnerId: frame.winnerId, reason: "normal" })}>确认本局并进入下一局</button>}
    {isHost && <details className="realtime-score-tools"><summary>裁判操作</summary><div className="realtime-tool-grid"><div><b>比赛状态</b><button disabled={busy} onClick={() => onCommand(match.pausedAt ? "snooker.resume" : "snooker.pause", {})}>{match.pausedAt ? "恢复比赛" : "暂停比赛"}</button><button disabled={disabled || frame.status !== "active"} onClick={() => onCommand("snooker.frame.restart", {})}>僵局重开本局</button></div><div><b>自由球</b><label>指定球<select value={freeBall} onChange={(event) => setFreeBall(event.target.value as SnookerBall)}>{REALTIME_SNOOKER_BALLS.map(([ball]) => <option value={ball} key={ball}>{REALTIME_SNOOKER_LABELS[ball]}</option>)}</select></label><label>作为目标球<select value={freeBallAs} onChange={(event) => setFreeBallAs(event.target.value as SnookerBall)}>{REALTIME_SNOOKER_BALLS.map(([ball]) => <option value={ball} key={ball}>{REALTIME_SNOOKER_LABELS[ball]}</option>)}</select></label><button disabled={disabled || frame.status !== "active"} onClick={() => onCommand("snooker.free_ball.declare", { nominatedBall: freeBall, valueAs: freeBallAs })}>声明自由球</button></div><div><b>犯规与重打</b><button disabled={disabled || frame.status !== "active"} onClick={() => onCommand("snooker.foul.record", { values: [4], isMiss: true })}>犯规 4 分并判 miss</button><button disabled={disabled || !frame.lastFoul?.isMiss} onClick={() => onCommand("snooker.replay.request", { kind: "from_position" })}>从现位重打</button><button disabled={disabled || !frame.lastFoul?.isMiss} onClick={() => onCommand("snooker.replay.request", { kind: "restore" })}>复原后重打</button></div><div><b>判局与黑球</b>{frame.phase === "final_black" && frame.scores[state.players[0].id] === frame.scores[state.players[1].id] && state.players.map((player) => <button key={player.id} disabled={disabled} onClick={() => onCommand("snooker.respotted_black.start", { firstStrikerId: player.id })}>重置黑球：{player.nickname}先手</button>)}{state.players.map((player) => <button key={player.id} disabled={disabled} onClick={() => onCommand("snooker.frame.finish", { winnerId: player.id, reason: "resignation" })}>认输／判{player.nickname}胜</button>)}</div></div></details>}
    {!writable && <p className="readonly-hint">观战者只读；玩家仅可记录自己回合的合法进球与结束本杆。</p>}
  </section>;
}
...
function RealtimeCardPanel({
  ...
  return <section className="match-section card-board realtime-card-panel">
    ...
    {visiblePlayers.length > 1 && <div className="hand-tabs">{visiblePlayers.map((player) => <button key={player.id} className={activePlayer?.id === player.id ? "active" : ""} aria-pressed={activePlayer?.id === player.id} onClick={() => setActivePlayerId(player.id)}>{player.nickname}<small>{cards.hands[player.id]?.length ?? 0} 张</small></button>)}</div>}
...
function RealtimeRoomPanel({ user, roomCode = "", onNavigate }: { user: AuthUser; roomCode?: string; onNavigate: (path: string) => void }) {
  ...
  const [snookerLeadNotice, setSnookerLeadNotice] = useState<RealtimeSnookerLeadNotice | null>(null);
  ...
  const announcedSnookerLead = useRef("");
  ...
  useEffect(() => {
    const snooker = snapshot?.snooker;
    const frame = snooker?.match.currentFrame;
    if (!snooker || !frame || frame.status !== "active") { announcedSnookerLead.current = ""; return; }
    const situation = getSnookerScoreSituation(frame);
    if (!situation.leaderId || !situation.trailingId || situation.excessPoints <= 0) { announcedSnookerLead.current = ""; return; }
    const noticeKey = `${snapshot.matchId}:${frame.id}:${situation.leaderId}`;
    if (announcedSnookerLead.current === noticeKey) return;
    announcedSnookerLead.current = noticeKey;
    const leaderName = snooker.players.find((player) => player.id === situation.leaderId)?.nickname ?? "领先选手";
    const trailingName = snooker.players.find((player) => player.id === situation.trailingId)?.nickname ?? "落后选手";
    setSnookerLeadNotice({ frameId: frame.id, frameNumber: frame.number, leaderName, trailingName, lead: situation.lead, remainingPoints: situation.remainingPoints, excessPoints: situation.excessPoints });
  }, [snapshot]);
          ...
          if (payload.type === "event") {
            const notice = payload.event && createRealtimeCardNotice(payload.event as { kind?: string; payload?: Record<string, unknown> }, (playerId) => {
              ...
              return current?.chaseScore?.players.find((player) => player.id === playerId)?.nickname
                ...
                ?? current?.snooker?.players.find((player) => player.id === playerId)?.nickname
                ?? playerId;
            });
  ...
  const liveSnookerFrame = snapshot?.snooker?.match.currentFrame;
  const showSnookerLeadNotice = Boolean(snookerLeadNotice && liveSnookerFrame?.id === snookerLeadNotice.frameId
    && liveSnookerFrame.status === "active" && getSnookerScoreSituation(liveSnookerFrame).excessPoints > 0);

  return <div className="room-page page-shell">
    {showSnookerLeadNotice && snookerLeadNotice && <RealtimeSnookerLeadDialog notice={snookerLeadNotice} onDismiss={() => setSnookerLeadNotice(null)} />}
    {cardNotice && <div className="card-notice-layer">
      <button type="button" className="card-notice-backdrop" aria-label="关闭卡牌通知" onClick={() => setCardNotice(null)} />
      <section className={`card-notice ${cardNotice.action}`} role="status" aria-live="polite" aria-atomic="true">
...
function CloudMatchesPanel({ ensureDevice, onRestore }: { ensureDevice: () => Promise<string>; onRestore: (match: CloudMatchSnapshot, readOnly: boolean) => void }) {
  const [matches, setMatches] = useState<CloudMatchRow[]>([]);
...
function AccountDataDialog({ onClose, onDeleted }: { onClose: () => void; onDeleted: () => void }) {
  ...
  return <dialog ref={dialogRef} className="setup-modal account-data-dialog" aria-labelledby="account-data-title" onCancel={(event) => { event.preventDefault(); if (!busy) onClose(); }}><header className="modal-heading"><div><p className="kicker">DATA CONTROL</p><h2 id="account-data-title">导出与删除</h2></div><button type="button" className="icon-button" aria-label="关闭导出与删除弹窗" disabled={busy} onClick={onClose}>×</button></header><div className="setup-body account-data-panel"><div className="data-tool-grid"><div className="data-tool-card"><b>完整账号数据</b><small>导出为便于阅读和处理的多行 JSON 文件。</small><button className="secondary" disabled={busy} onClick={() => void exportData()}>导出完整账号 JSON</button></div><div className="data-tool-card json-report-tool"><b>战绩 JSON 转长图 / PDF</b><small>文件仅在本机浏览器中读取，不上传服务器、不写入数据库。</small><label className="json-file-button">上传 JSON 文件<input name="match-json-file" type="file" autoComplete="off" accept="application/json,.json" onChange={(event) => void loadReportFile(event.target.files?.[0])} /></label><label htmlFor="match-json-report">粘贴 JSON 内容</label><textarea id="match-json-report" name="match-json-report" autoComplete="off" value={reportJson} spellCheck={false} placeholder={'粘贴导出的战绩 JSON，例如：\n{\n  "exportVersion": 1,\n  "match": {}\n}\n…'} aria-describedby={reportError ? "match-json-error" : undefined} onChange={(event) => { setReportJson(event.target.value); setReportError(""); }} onBlur={() => { if (reportJson.trim()) { try { parseMatchReport(reportJson); } catch (error) { setReportError(error instanceof Error ? error.message : "JSON 格式无效"); } } }} /><ReportOptionFields options={reportOptions} onChange={setReportOptions} /><div className="json-report-actions"><button className="secondary" disabled={!!converting || !reportJson.trim()} onClick={() => void exportReport("png")}>{converting === "png" ? "正在生成长图…" : "下载长图 PNG"}</button><button className="secondary" disabled={!!converting || !reportJson.trim()} onClick={() => void exportReport("pdf")}>{converting === "pdf" ? "正在生成 PDF…" : "下载 PDF"}</button></div>{reportError && <p id="match-json-error" className="json-report-error" role="alert">{reportError}</p>}</div></div><details><summary>永久删除账号…</summary><p>删除会立即撤销全部会话并删除本人独有数据；已有其他注册参与者的共享对局会保留，但解除你的账号关联。</p><label>当前密码<input name="delete-account-password" type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label><label>输入“删除账号”确认<input name="delete-account-confirmation" autoComplete="off" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label><button className="danger-button" disabled={busy || confirmation !== "删除账号" || !password} onClick={() => void deleteAccount()}>永久删除账号</button></details>{message && <p className="form-message" role="status">{message}</p>}</div></dialog>;
}

function AccountSecurityDialog({ forced = false, onClose, onChanged }: { forced?: boolean; onClose: () => void; onChanged?: () => void }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  ...
  const changePassword = async () => {
    ...
    setBusy(true); setMessage("");
    try {
      await apiPayload(await fetch("/api/auth/change-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ currentPassword, newPassword }),
      }));
      setCurrentPassword(""); setNewPassword(""); setConfirmation("");
      setMessage("密码已更新，其他设备上的登录已失效");
      onChanged?.();
    } catch (error) { setMessage(error instanceof Error ? error.message : "密码修改失败"); }
    finally { setBusy(false); }
  };
  return <dialog ref={dialogRef} className="setup-modal account-security-dialog" aria-labelledby="change-password-title" onCancel={(event) => { event.preventDefault(); if (!forced) onClose(); }}><header className="modal-heading"><div><p className="kicker">ACCOUNT SECURITY</p><h2 id="change-password-title">{forced ? "请先更改临时密码" : "更改密码"}</h2></div>{!forced && <button type="button" className="icon-button" aria-label="关闭更改密码弹窗" onClick={onClose}>×</button>}</header><form onSubmit={(event) => { event.preventDefault(); void changePassword(); }}><div className="setup-body account-security-form"><label>旧密码<input name="current-password" type="password" autoComplete="current-password" minLength={6} maxLength={64} required value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} /></label><label>新密码<input name="new-password" type="password" autoComplete="new-password" minLength={6} maxLength={64} required value={newPassword} onChange={(event) => setNewPassword(event.target.value)} /></label><label>确认新密码<input name="confirm-new-password" type="password" autoComplete="new-password" minLength={6} maxLength={64} required value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label><small>{forced ? "管理员已将密码重置为 123456。完成改密后才能继续使用账号功能。" : "忘记旧密码？请联系管理员将密码重置为 123456，登录后再立即更改。"}</small>{message && <p className="form-message" role="status">{message}</p>}</div><footer className="modal-actions">{!forced && <button type="button" className="secondary" disabled={busy} onClick={onClose}>取消</button>}<button className="primary" disabled={busy}>{busy ? "正在更新…" : "确认更改密码"}</button></footer></form></dialog>;
}

function ProfilePage({ history, user, authLoading, sync, onAuthenticated, onPasswordChanged, onLogout, onAccountDeleted, onRetrySync, ensureDevice, onRestore, onNavigate }: { history: BilliardsMatch[]; user: AuthUser | null; authLoading: boolean; sync: SyncView; onAuthenticated: (user: AuthUser) => void; onPasswordChanged: () => void; onLogout: () => void; onAccountDeleted: () => void; onRetrySync: () => void; ensureDevice: () => Promise<string>; onRestore: (match: CloudMatchSnapshot, readOnly: boolean) => void; onNavigate: (path: string) => void }) {
  const [passwordOpen, setPasswordOpen] = useState(false);
  const forcedPasswordChange = !!user?.mustChangePassword;
  return <div className="content-page page-shell"><header className="profile-hero"><span className="profile-avatar">{user?.nickname.slice(0, 1) || "游"}</span><div><p className="kicker">{user ? `CLOUD ACCOUNT · ${user.publicCode}` : `LOCAL GUEST · V${APP_VERSION}`}</p><h1>{user ? user.nickname : "游客模式"}</h1><p>{user ? `@${user.username}` : "无需注册也可继续计分；登录后才会补传和跨设备恢复。"}</p></div>{user && <div className="profile-account-actions"><button className="secondary" onClick={onLogout}>退出账号</button><button className="secondary" onClick={() => setPasswordOpen(true)}>更改密码</button></div>}</header>{authLoading ? <section className="account-panel">正在检查账号会话…</section> : !user && <AccountForm onAuthenticated={onAuthenticated} />}<section className="local-stats"><div><strong>{history.length}</strong><span>本机已完成</span></div><div><strong>{history.reduce((sum, match) => sum + match.scoreEvents.length, 0)}</strong><span>计分流水</span></div><div><strong>{sync.pending}</strong><span>待补传项目</span></div></section><section className="settings-list"><header><p className="kicker">SYNC STATUS</p><h2>本机与云端</h2></header><div><span>◎</span><p><b>本地自动保存</b></p><strong className="state-good">已开启</strong></div><div><span>⇅</span><p><b>云端同步</b><small>{sync.message}</small></p><strong className={`sync-label ${sync.state}`}>{({ local: "仅本地", pending: "待同步", syncing: "同步中", synced: "已同步", failed: "同步失败", readonly: "只读" } as const)[sync.state]}</strong></div>{user && (sync.state === "failed" || sync.state === "pending") && <div className="settings-action"><button className="secondary" onClick={onRetrySync}>手动重试</button></div>}</section>{user && <><section className="realtime-entry-card"><div><p className="kicker">REALTIME ROOM</p><h2>多人实时房间</h2><small>创建或加入云端实时房间，全屏共同操作。</small></div><SpaLink className="primary" href="/room" navigate={onNavigate}>进入实时房间 <span>→</span></SpaLink></section><CloudMatchesPanel ensureDevice={ensureDevice} onRestore={onRestore} /><LocalMigrationPanel onAccountDeleted={onAccountDeleted} />{(forcedPasswordChange || passwordOpen) && <AccountSecurityDialog forced={forcedPasswordChange} onClose={() => setPasswordOpen(false)} onChanged={onPasswordChanged} />}</>}</div>;
}

function ConfirmDialog({ title, body, cancelLabel = "继续对局", confirmLabel = "确认结束并保存", busy = false, error = "", onCancel, onConfirm }: { title: string; body: string; cancelLabel?: string; confirmLabel?: string; busy?: boolean; error?: string; onCancel: () => void; onConfirm: () => void }) {
  const dialogRef = useModalDialog(busy ? undefined : onCancel);
  return <dialog ref={dialogRef} className="confirm-modal" role="alertdialog" aria-labelledby="confirm-dialog-title" onCancel={(event) => { event.preventDefault(); if (!busy) onCancel(); }}><span className="warning-icon" aria-hidden="true">!</span><h2 id="confirm-dialog-title">{title}</h2><p>{body}</p>{error && <p className="form-message" role="alert">{error}</p>}<div className="modal-actions"><button className="secondary" disabled={busy} onClick={onCancel}>{busy ? "正在处理…" : cancelLabel}</button><button className="danger-button" disabled={busy} onClick={onConfirm}>{busy ? "正在处理…" : confirmLabel}</button></div></dialog>;
}
...
function DeleteMatchDialog({ label, players, time, busy, error, onCancel, onConfirm }: { label: string; players: string; time: string; busy: boolean; error: string; onCancel: () => void; onConfirm: () => void }) {
  const dialogRef = useModalDialog(busy ? undefined : onCancel);
  return <dialog ref={dialogRef} className="confirm-modal" role="alertdialog" aria-labelledby="delete-match-title" onCancel={(event) => { event.preventDefault(); if (!busy) onCancel(); }}><span className="warning-icon" aria-hidden="true">!</span><h2 id="delete-match-title">删除这条战绩？</h2><p><b>{label}</b> · {players}<br /><small>{time}</small><br />删除后将从当前账号和本机战绩中移除；共享对局仍会为其他参与者保留。此操作不可恢复。</p>{error && <p className="form-message" role="alert">{error}</p>}<div className="modal-actions"><button className="secondary" disabled={busy} onClick={onCancel}>{busy ? "正在删除…" : "取消"}</button><button className="danger-button" disabled={busy} onClick={onConfirm}>确认删除</button></div></dialog>;
}
...
function ActiveMatchProtectionDialog({ match, discardArmed, onContinue, onSave, onArmDiscard, onDiscard }: { match: BilliardsMatch; discardArmed: boolean; onContinue: () => void; onSave: () => void; onArmDiscard: () => void; onDiscard: () => void }) {
  const dialogRef = useModalDialog(onContinue);
  return <dialog ref={dialogRef} className="confirm-modal protection-modal" role="alertdialog" aria-labelledby="protect-title" onCancel={(event) => { event.preventDefault(); onContinue(); }}><span className="warning-icon" aria-hidden="true">!</span><h2 id="protect-title">发现未结束对局</h2><p>{match.players.map((player) => player.name).join("、")} 的对局仍在进行。新建前请选择如何处理，旧对局不会被静默覆盖。</p><div className="protection-actions"><button className="primary" onClick={onContinue}>继续当前对局</button><button className="secondary" onClick={onSave}>保存当前对局后新建</button>{discardArmed ? <button className="danger-button" onClick={onDiscard}>再次确认：放弃并新建</button> : <button className="text-button danger-text" onClick={onArmDiscard}>放弃旧对局…</button>}</div>{discardArmed && <small className="audit-note">确认后仍会保留可恢复快照和放弃时间。</small>}</dialog>;
}

function StorageRecoveryDialog({ issue, onRetry, onReset }: { issue: StorageIssue; onRetry: () => void; onReset: () => void }) {
  const dialogRef = useModalDialog();
  return <dialog ref={dialogRef} className="confirm-modal protection-modal" role="alertdialog" aria-labelledby="recovery-title" onCancel={(event) => event.preventDefault()}><span className="warning-icon" aria-hidden="true">!</span><h2 id="recovery-title">本机数据无法读取</h2><p>原因：{issue.message}。为防止静默丢失，应用已暂停写入。你可以重试，或先备份原始数据再安全重置。</p><div className="protection-actions"><button className="primary" onClick={onRetry}>重试读取</button><button className="danger-button" onClick={onReset}>备份并安全重置</button></div></dialog>;
}
...
export default function GameApp() {
  ...
  const completeSnooker = () => {
    if (!data.activeSnookerMatch) return;
    const completed = recordSnookerCommand(data.activeSnookerMatch, { type: "snooker.finish" });
    setData({ ...data, activeSnookerMatch: null, snookerHistory: [completed, ...data.snookerHistory] });
    setConfirmEnd(false);
    navigate(`/history/${completed.id}`);
    setStatus("斯诺克比赛已结束，比分与单杆战绩已保存");
  };
  ...
  const restoreCloudMatch = (match: CloudMatchSnapshot, readOnly: boolean) => {
    setData((current) => {
      if (isSnookerMatch(match)) {
        return match.status === "completed"
          ? { ...current, snookerHistory: [match, ...current.snookerHistory.filter((item) => item.id !== match.id)] }
          : { ...current, activeSnookerMatch: match };
      }
  ...
  const deleteMatchRecord = async (id: string) => {
    ...
    setData((current) => ({
      ...
      snookerHistory: current.snookerHistory.filter((match) => match.id !== id),
    }));
    const store = browserStore();
  ...
  const page = (() => {
    if (path === "/") return data.activeSnookerMatch
      ...
      : data.activeMatch
        ...
        : <div className="home-stack">
            <EmptyHome onStart={openSetup} onStartEight={openEightSetup} onStartSnooker={openSnookerSetup} onStartTeamBattle={openTeamBattleSetup} onNavigate={navigate} onResume={resumePaused} recent={data.history[0]} paused={data.pausedMatches} user={user} onEnterCloudRoom={enterCloudRoom} />
          </div>;
    ...
    if (path === "/room" || path.startsWith("/room/")) {
      if (!user) return (
        <div className="room-page page-shell">
          ...
          <div className="room-entry-grid"><div><b>需要登录</b><small>登录后可创建房间并邀请朋友用房间码加入，全屏共同操作。</small><SpaLink className="primary" href="/profile" navigate={navigate}>前往登录 <span>→</span></SpaLink></div></div>
        </div>
      );
    ...
    if (path === "/profile") return <ProfilePage history={data.history} user={user} authLoading={authLoading} sync={sync} onAuthenticated={(nextUser) => { setUser(nextUser); setStatus("账号已连接，正在检查离线队列"); }} onPasswordChanged={() => setUser((current) => current ? { ...current, mustChangePassword: false } : current)} onLogout={() => void logout()} onAccountDeleted={() => { setUser(null); setSync(LOCAL_SYNC_VIEW); setStatus("云端账号已删除；本机数据仍保留"); }} onRetrySync={() => void runSync(true)} ensureDevice={ensureDevice} onRestore={restoreCloudMatch} onNavigate={navigate} />;
  ...
  return (
    <div className="app-root">
      ...
      {status && <div className="status-toast" role="status"><span aria-hidden="true">✓</span>{status}</div>}
    </div>
  );
}
