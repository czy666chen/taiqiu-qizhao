  ...
  return (
    <div className="home-page page-shell">
      ...
      <section className="quick-start" aria-labelledby="quick-start-title">
        <header className="quick-start-heading"><div><p className="kicker">CHOOSE YOUR GAME</p><h2 id="quick-start-title">今晚玩什么</h2></div><p>先选规则，再邀请球友。每种玩法都保留完整流水。</p></header>
...
function AccountSecurityDialog({ onClose }: { onClose: () => void }) {
  ...
  return <dialog ref={dialogRef} className="setup-modal account-security-dialog" aria-labelledby="change-password-title" onCancel={(event) => { event.preventDefault(); onClose(); }}><header className="modal-heading"><div><p className="kicker">ACCOUNT SECURITY</p><h2 id="change-password-title">更改密码</h2></div><button type="button" className="icon-button" aria-label="关闭更改密码弹窗" onClick={onClose}>×</button></header><form onSubmit={(event) => { event.preventDefault(); void changePassword(); }}><div className="setup-body account-security-form"><label>旧密码<input name="current-password" type="password" autoComplete="current-password" minLength={6} maxLength={64} required value={currentPassword} onChange={(event) => setCurrentPassword(event.target.value)} /></label><label>新密码<input name="new-password" type="password" autoComplete="new-password" minLength={6} maxLength={64} required value={newPassword} onChange={(event) => setNewPassword(event.target.value)} /></label><label>确认新密码<input name="confirm-new-password" type="password" autoComplete="new-password" minLength={6} maxLength={64} required value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /></label><small>忘记旧密码？请联系管理员将密码重置为 123456，登录后再立即更改。</small>{message && <p className="form-message" role="status">{message}</p>}</div><footer className="modal-actions"><button type="button" className="secondary" disabled={busy} onClick={onClose}>取消</button><button className="primary" disabled={busy}>{busy ? "正在更新…" : "确认更改密码"}</button></footer></form></dialog>;
}
