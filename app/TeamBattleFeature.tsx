...
export function TeamBattleSetupDialog({ onClose, onStart }: { onClose: () => void; onStart: (draft: TeamBattleDraft) => void }) {
  ...
  return <dialog ref={dialogRef} className="setup-modal team-battle-setup" aria-labelledby="team-battle-setup-title" onCancel={(event) => { event.preventDefault(); onClose(); }}>
    ...
    <div className="setup-body">
      ...
      <section className="setup-section"><div className="setup-title"><span>01</span><div><b>在场成员</b><small>2–8 人，仅保存在本机；可在比赛中继续加入</small></div></div>
        <div className="team-setup-players">{names.map((name, index) => {
          ...
          return <div key={index} className="team-setup-player">
            ...
            <input id={`team-player-${index}`} autoComplete="off" maxLength={TEAM_BATTLE_MAX_PLAYER_NAME_LENGTH} value={name} aria-describedby={fieldError ? errorId : undefined} onBlur={() => setTouched({ ...touched, [index]: true })} onChange={(event) => setNames(names.map((item, itemIndex) => itemIndex === index ? event.target.value : item))} />
      ...
      <section className="setup-section"><div className="setup-title"><span>02</span><div><b>比赛信息</b><small>均为可选项</small></div></div><div className="eight-form-grid"><label className="wide"><span>比赛标题</span><input maxLength={40} value={title} onChange={(event) => setTitle(event.target.value)} /></label><label><span>地点</span><input maxLength={40} value={location} onChange={(event) => setLocation(event.target.value)} /></label><label className="wide"><span>备注</span><input maxLength={120} value={note} onChange={(event) => setNote(event.target.value)} /></label></div></section>
    </div>
...
export function TeamBattleBoard({ match, onChange, onFinish, toast }: { match: TeamBattleMatch; onChange: (match: TeamBattleMatch) => void; onFinish: () => void; toast: (message: string) => void }) {
  ...
  const editRound = (round: EffectiveTeamBattleRound) => {
    ...
    document.querySelector(".team-round-panel")?.scrollIntoView({ behavior: "smooth" });
  };
  ...
  return <div className="eight-page team-battle-page page-shell">
    ...
    <section className="eight-round-panel team-round-panel"><div className="section-heading"><div><p className="kicker">{editingEventId ? "CORRECTION" : `ROUND ${projection.rounds.length + 1}`}</p><h2>{editingEventId ? "更正本局结果" : "记录本局结果"}</h2></div><button className="text-button" disabled={!currentPair.rounds.length} onClick={() => update(() => undoLastTeamBattleRound(match, pair), "已撤销当前组合的上一局")}>↶ 撤销当前组合上一局</button></div>
      ...
      <div className="eight-fouls">{pair.map((id) => <label key={id}><span>{playerName(id)} 本局犯规</span><input type="number" min="0" inputMode="numeric" value={fouls[id] ?? 0} onChange={(event) => setFouls({ ...fouls, [id]: Number(event.target.value) })} /></label>)}</div>
      <label className="score-note"><span>本局备注</span><input maxLength={120} placeholder="可选" value={note} onChange={(event) => setNote(event.target.value)} /></label>
...
export function TeamBattleHistoryDetail({ match, onBack, onDelete }: { match: TeamBattleMatch; onBack: () => void; onDelete: (id: string) => void }) {
  ...
  return <div className="content-page page-shell team-battle-history">
    <button className="back-link" onClick={onBack}>← 返回战绩</button>
