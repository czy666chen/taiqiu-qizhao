...
test("切换明暗主题会同步浏览器主题色", async ({ page }) => {
  await page.goto("/");
  await expect(page.locator('meta[name="theme-color"]')).toHaveAttribute("content", "#07110d");
  await expect(page.locator("html")).toHaveCSS("background-color", "rgb(7, 17, 13)");
  await page.getByRole("button", { name: "切换到白天版本" }).click();
  await expect(page.locator('meta[name="theme-color"]')).toHaveAttribute("content", "#f3f0e7");
  await expect(page.locator("html")).toHaveAttribute("data-theme", "day");
  await expect(page.locator("html")).toHaveCSS("background-color", "rgb(243, 240, 231)");
